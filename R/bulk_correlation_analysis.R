...
compute_corr <- function(df, i, v1, v2, method = "pearson") {
    ...
    return(
        cor(
            x = df_sub %>% dplyr::pull(!!dplyr::sym(v1)),
            y = df_sub %>% dplyr::pull(!!dplyr::sym(v2)),
            method = method
            # use = "complete.obs"
        )
    )
...
#' @param method method to use for computing correlation
#' @export
run_bootstrap_corr <- function(pair, df, stat_func = compute_corr, n_iter = 500, method = "pearson", ncpus = 1) {
    ...
    bootcorr <- boot::boot(df, statistic = stat_func, R = n_iter, v1 = v1, v2 = v2, method = method, parallel = "multicore", ncpus = ncpus)
